...
const EditCouponModal = ({ couponToEdit, onClose, onSave, isSubmitting, brandColor }) => {
  ...
  useEffect(() => {
    if (couponToEdit) {
      ...
      const date = couponToEdit.expiryDate ? new Date(couponToEdit.expiryDate) : null;
  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <input type="text" placeholder="Coupon Code Name" className="w-full px-4 py-3 border border-gray-300 rounded-lg" value={couponCodeName} onChange={(e) => setCouponCodeName(e.target.value)} />
        </div>
        <div className="relative">
          <input type="number" placeholder="Percentage off" className="w-full px-4 py-3 border border-gray-300 rounded-lg" value={percentageOff} onChange={(e) => setPercentageOff(e.target.value)} min="1" max="100" />
          <span className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-500 font-semibold">%</span>
        </div>
        <div>
          <input type="number" placeholder="Maximum Usage" className="w-full px-4 py-3 border border-gray-300 rounded-lg" value={maxUsage} onChange={(e) => setMaxUsage(e.target.value)} min="1" />
        </div>
        <div>
          <input type="number" placeholder="No of usage / User" className="w-full px-4 py-3 border border-gray-300 rounded-lg" value={usagePerUser} onChange={(e) => setUsagePerUser(e.target.value)} min="1" />
        </div>
        <div>
          <label htmlFor="editExpiryDate" className="block text-sm font-medium text-gray-700 mb-1">Expiry date</label>
          <input type="date" id="editExpiryDate" className="w-full px-4 py-3 border border-gray-300 rounded-lg" value={expiryDate} onChange={(e) => setExpiryDate(e.target.value)} />
        </div>
