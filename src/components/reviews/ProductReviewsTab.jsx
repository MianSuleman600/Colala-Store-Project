      ...
      {(!reviews || reviews.length === 0) ? (
        ...
        reviews.map((review) => (
          <Card key={review.id} className="p-4 rounded-xl shadow-sm flex flex-col">
            ...
            {Array.isArray(review.productImages) && review.productImages.length > 0 && (
              <div className="flex space-x-2 mb-3 overflow-x-auto pb-2">
                {review.productImages.map((img, index) => (
                  <img
                    ...
                    onError={(e) => {
                      e.currentTarget.onerror = null;
                      e.currentTarget.src = 'https://placehold.co/80x80/e0e0e0/000000?text=Img';
                    }}
                  />
                ))}
              </div>
            ...
            <div className="flex justify-end">
              <button
                onClick={() => onViewReview?.({ ...review, type: 'product' })}
