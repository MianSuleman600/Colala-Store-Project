...
const OrderTrackingStep = ({
  ...
  brandColor,
  contrastTextColor,
  showMarkAsOutForDeliveryButton = false,
      ...
      <Card className={`flex-grow p-4 rounded-xl shadow-sm mb-6 ${isActive ? 'bg-white' : 'bg-gray-50'}`}>
        ...
        <div className="relative flex items-start mb-4 p-4  rounded-lg">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">Qty: {item?.qty ?? 0}</p>
          </div>

          {/* Status date at bottom-right */}
        ...
        {/* The button rendering logic below is correct and does not need changes. */}

        {title === 'Out for Delivery' && (
          <>
            {showMarkAsOutForDeliveryButton && (
              <Button
                ...
                disabled={isLoading}
              >
                {isLoading ? 'Updating...' : 'Mark as out for delivery'}
              </Button>
            )}
            {isActive && !showMarkAsOutForDeliveryButton && (
              <div
        ...
        {title === 'Delivered' && (
          <>
            ...
            {isActive && !showRequestCodeButton && (
              <div
                className="w-full py-2 px-4 rounded-lg text-sm font-medium flex items-center justify-center opacity-70"
                style={{ backgroundColor: brandColor, color: contrastTextColor }}
              >
                <CheckCircleIcon className="h-4 w-4 mr-2 inline-block" /> Delivered
              </div>
            )}
          </>
        )}
        ...
        {title === 'Funds Released' && (
          <>
            ...
            {isActive && !showViewWalletButton && (
              <div
                className="w-full py-2 px-4 rounded-lg text-sm font-medium flex items-center justify-center opacity-70"
                style={{ backgroundColor: brandColor, color: contrastTextColor }}
              >
                <CheckCircleIcon className="h-4 w-4 mr-2 inline-block" /> Funds Released
              </div>
            )}
          </>
        )}

        {title === 'Order Completed' && isActive && (
          <div
            className="w-full py-2 px-4 rounded-lg text-sm font-medium flex items-center justify-center opacity-70"
            style={{ backgroundColor: brandColor, color: contrastTextColor }}
          >
