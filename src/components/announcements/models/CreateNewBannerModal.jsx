...
const CreateNewBannerModal = ({ onClose, onSave, brandColor }) => {
  ...
  const [placement, setPlacement] = useState('home'); // 'home' | 'profile'
  const [alt, setAlt] = useState('Promotional Banner');
  ...
  const canPreview = useMemo(() => {
    return Boolean(previewUrl || (imageUrl && isValidUrl(imageUrl)));
  }, [previewUrl, imageUrl]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Payload: allow file upload if backend supports it; otherwise imageUrl
    const payload = {
      imageUrl: imageUrl || previewUrl, // Use the provided URL, else use preview for immediate UI
      link: link ? (link.startsWith('http') ? link : `https://${link}`) : '',
      placement,
      alt: alt || 'Promotional Banner',
      active,
      // Optional: pass file for services that support multipart uploads
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-800 font-serif">New Banner</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">Optional. If provided, clicking the banner opens this URL.</p>
        </div>
