...
const AddProductPage = () => {
  ...
  const handleAddVariant = () => {
    ...
    const optionsArray = currentVariantOptions
      .split(',')
      .map((o) => o.trim())
      .filter(Boolean);
    ...
    setFormData((prev) => ({
      ...
      variants: [
        ...prev.variants,
        { name: currentVariantName.trim(), options: optionsArray },
      ],
    }));
  ...
  const selectedCategoryName =
    categories.find((c) => c.id === formData.category)?.title || 'Category';
    ...
    <div className="p-4">
      {isLoading ? (
        ...
        <p className="text-gray-500">
          No locations found. Please add them in your store setup first.
        </p>
      ) : (
        <ul className="space-y-2">
          {locationList.map((loc) => (
            <li
              key={loc}
              className="p-2 flex items-center justify-between rounded-lg hover:bg-gray-100"
            >
              <span>{loc}</span>
              <input
                type="checkbox"
                checked={formData[type].includes(loc)}
                onChange={() => handleToggleLocation(loc, type)}
          ...
          >
            <ProductDetailsForm
              ...
              renderFullDescriptionSummary={() =>
                'Click to add/edit detailed specifications'
              }
