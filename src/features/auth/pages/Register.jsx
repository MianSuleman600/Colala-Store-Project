...
const Register = ({ onClose, onSwitchToLogin, mode = "register" }) => {
  ...
  const { formData, currentLevel, currentStep } = useSelector(
    (s) => s.registration
  );
  ...
  const { mutateAsync: startRegistration } =
    useStartSellerRegistrationMutation();
  const { mutateAsync: submitL1ProfileMedia } =
    useSubmitL1ProfileMediaMutation();
  const { mutateAsync: submitL1CategoriesSocial } =
    useSubmitL1CategoriesSocialMutation();
  const { mutateAsync: submitL2BusinessDetails } =
    useSubmitL2BusinessDetailsMutation();
  const { mutateAsync: submitL2Documents } = useSubmitL2DocumentsMutation();
  const { mutateAsync: submitL3PhysicalStore } =
    useSubmitL3PhysicalStoreMutation();
  ...
  const handleStep1Submit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        store_location: stepData.storeLocation,
        store_email: stepData.email,
        store_phone: stepData.phoneNumber,
        password: stepData.password,
        referral_code: stepData.referralCode,
      };
      if (mode === "register") {
        ...
        if (response.token) {
          ...
          dispatch(
            loginSuccess({ token: response.token, user: temporaryUser })
          );
          dispatch(setLevelStep({ level: 1, step: 2 }));
          push("Registration successful!", { type: "success" });
        } else {
          console.error(
            "❌ Registration API did not return a token.",
            response
          );
      ...
      if (error?.response?.data?.message?.toLowerCase().includes("email")) {
        push("Email already exists. Please use a different email.", {
          type: "error",
        });
      } else {
        push(error.message || "Step 1 submission failed. Please try again.", {
          type: "error",
        });
      }
    } finally {
  ...
  const handleStep2Submit = async () => {
    ...
    try {
      ...
      if (stepData.profilePicture)
        apiFormData.append("profile_image", stepData.profilePicture);
      if (stepData.storeBanner)
        apiFormData.append("banner_image", stepData.storeBanner);
  ...
  const handleStep5Submit = async () => {
    ...
    try {
      ...
      const apiFormData = new FormData();
      if (stepData.ninSlip)
        apiFormData.append("nin_document", stepData.ninSlip);
      if (stepData.cacCertificate)
        apiFormData.append("cac_document", stepData.cacCertificate);
  ...
  const handleStep6Submit = async () => {
    ...
    try {
      ...
      const dataPayload = {
        has_physical_store: stepData.hasPhysicalStore ? 1 : 0,
      };
  ...
  const handleStep7Submit = async () => {
    ...
    try {
      ...
      if (stepData.selectedColor) {
        promisesToRun.push(
          submitL3Theme({ theme_color: stepData.selectedColor })
        );
      }
  ...
  const renderCurrentForm = () => {
    ...
    if (currentLevel === 1) {
      ...
      return (
        <Level1Form
          ...
          formData={formData.level1[stepKey]}
          handleChange={(e) => handleChange(e, levelKey, stepKey)}
          handleFileChange={(e) => handleFileChange(e, levelKey, stepKey)}
        />
      );
    }
    if (currentLevel === 2) {
      let onNextHandler =
        currentStep === 1 ? handleStep4Submit : handleStep5Submit;
      return (
        <Level2Form
          {...commonProps}
          onNext={onNextHandler}
          activeStep={currentStep}
          formData={formData.level2[stepKey]}
          handleChange={(e) => handleChange(e, levelKey, stepKey)}
          handleFileChange={(e) => handleFileChange(e, levelKey, stepKey)}
        />
      );
    ...
    if (currentLevel === 3) {
      let onNextHandler =
        currentStep === 1 ? handleStep6Submit : handleStep7Submit;
      return (
        <Level3Form
          {...commonProps}
          onNext={onNextHandler}
          activeStep={currentStep}
          formData={formData.level3[stepKey]}
          handleChange={(e) => handleChange(e, levelKey, stepKey)}
          handleFileChange={(e) => handleFileChange(e, levelKey, stepKey)}
        />
      );
          ...
          <div className="absolute bottom-0 mb-5 p-8 bg-[#921313] bg-opacity-60 rounded-2xl w-3/4 h-[270px] backdrop-blur-sm">
            ...
            <ul className="space-y-2 text-sm mt-3">
              <li className="flex items-center">
                <CheckCircleIcon className="h-4 w-4 mr-2" /> Extra delivery
                zones
              </li>
              <li className="flex items-center">
            ...
            <Button className="mt-3 bg-white text-red-600 rounded-md py-1 px-4">
              Request Service
            </Button>
          </div>
        </div>
