...
const useGetCatalogCategoriesQuery = ({ enabled }) => useQuery({
...
const Level1Form = ({
    ...
    const { data: categoriesData, isLoading: isLoadingCategories } = useGetCatalogCategoriesQuery({
    ...
    const isStepValid = useMemo(() => {
        switch (activeStep) {
            case 1:
                ...
                const isPasswordValid = mode === 'register' ? (formData.password && formData.password.length >= 6) : true;
                return !!(formData.storeName && formData.storeLocation && formData.email && isEmailValid && formData.phoneNumber && isPasswordValid);
            case 2:
    ...
    const validateStep = () => {
        ...
        switch (activeStep) {
            case 1:
                ...
                if (!formData.storeLocation) newErrors.storeLocation = "Location is required.";
                if (!formData.email) newErrors.email = "Email is required.";
                else if (!/\S+@\S+\.\S+/.test(formData.email)) newErrors.email = "Email is invalid.";
                if (!formData.phoneNumber) newErrors.phoneNumber = "Phone number is required.";
                if (mode === 'register' && (!formData.password || formData.password.length < 6)) {
                    newErrors.password = "Password must be at least 6 characters.";
                }
                break;
            case 2:
                if (!formData.profilePicture) push('Profile picture is required.', { type: 'error' });
                if (!formData.storeBanner) push('Store banner is required.', { type: 'error' });
                break;
            case 3:
    ...
    const handleRemoveCategory = (categoryId) => {
        const updated = (formData.categories || []).filter((id) => id !== categoryId);
    ...
    const handleEmailSubmit = async (email) => {
        ...
        try {
            ...
            push(error.message || "Failed to send verification code.", { type: "error" });
        } finally {
    ...
    const handleOtpConfirm = async (otp) => {
        ...
        try {
            ...
            push(error.message || "Invalid OTP. Please try again.", { type: "error" });
        } finally {
    ...
    const handleNewPassword = async (newPassword) => {
        ...
        try {
            // Use your actual service method here
            await authService.resetPassword({ email: resetEmail, code: resetOtp, password: newPassword });
    ...
    const validateImage = (file) => {
        if (!ALLOWED_IMAGE_TYPES.includes(file.type)) {
            push(`Invalid file type. Please use ${ALLOWED_IMAGE_TYPES.join(", ")}.`, { type: 'error' });
            return false;
        }
        if (file.size > MAX_IMAGE_MB * 1024 * 1024) {
            push(`File is too large. Max size is ${MAX_IMAGE_MB}MB.`, { type: 'error' });
            return false;
        }
        return true;
    };
    ...
    const proceedButtonText = useMemo(() => {
        if (activeStep === 1) return mode === 'register' ? 'Create Account' : 'Proceed';
    ...
    return (
        ...
        <div className={`w-full h-full max-h-[90vh] ${mode === "register" ? "max-w-[389px] px-2 py-4 sm:px-8" : "max-w-none p-0"}`}>
            {/* Header */}
            ...
            <div className="space-y-4 mt-8 flex flex-col h-full">
                {activeStep === 1 && (
                    <>
                        ...
                        {mode === "register" && <Input type={showPassword ? "text" : "password"} name="password" placeholder="Password" icon={<LockClosedIcon className="h-5 w-5" />} value={formData.password || ""} onChange={handleChange} rightIcon={<button type="button" onClick={() => setShowPassword(!showPassword)}>{showPassword ? <EyeSlashIcon className="h-5 w-5" /> : <EyeIcon className="h-5 w-5" />}</button>} error={errors.password} />}
                        {mode === "upgrade" && <div className="rounded-[15px] p-3 bg-white border"><div className="flex items-center justify-between"><span className="text-sm font-medium">Change account password</span><Button type="button" onClick={handleOpenChangePassword} className="px-3 py-1 rounded-md text-sm" style={{ backgroundColor: brandColor, color: contrastColor }}>Change</Button></div><p className="text-xs text-gray-500 mt-2">We’ll send a verification code to your email.</p></div>}
                        <Input name="referralCode" placeholder="Referral Code (Optional)" value={formData.referralCode || ""} onChange={handleChange} />

                        <div className="mt-4 flex flex-col gap-2">
                            {mode === "upgrade" && <Button type="button" onClick={onSaveExit}>Save & Exit</Button>}
                            <Button type="button" onClick={handleProceed} className="w-full disabled:opacity-50 disabled:cursor-not-allowed" style={{ backgroundColor: brandColor, color: contrastColor }} disabled={!isStepValid}>
                                {proceedButtonText}
                            </Button>
                ...
                {activeStep === 2 && (
                    <>
                        <div className="space-y-2">
                            ...
                            <div role="button" tabIndex={0} onClick={() => profileInputRef.current?.click()} className="mx-auto h-32 w-32 rounded-full flex items-center justify-center overflow-hidden bg-gray-50 border">
                                {profilePreview ? <img src={profilePreview} alt="Profile Preview" className="h-full w-full object-cover" /> : <CameraIcon className="h-8 w-8 text-gray-400" />}
                            </div>
                            <input ref={profileInputRef} type="file" name="profilePicture" className="sr-only" accept={ALLOWED_IMAGE_TYPES.join(",")} onChange={onFileChange} />
                        </div>
                        <div className="space-y-2 mt-6">
                            <label className="block text-sm font-medium">Upload store banner</label>
                            <div role="button" tabIndex={0} onClick={() => bannerInputRef.current?.click()} className="w-full h-36 rounded-xl flex items-center justify-center overflow-hidden bg-gray-50 border">
                                {bannerPreview ? <img src={bannerPreview} alt="Banner Preview" className="h-full w-full object-cover" /> : <CameraIcon className="h-8 w-8 text-gray-400" />}
                            </div>
                            <input ref={bannerInputRef} type="file" name="storeBanner" className="sr-only" accept={ALLOWED_IMAGE_TYPES.join(",")} onChange={onFileChange} />
                        </div>
                        <div className="flex gap-4 mt-6">
                            <Button type="button" onClick={onBack}><ArrowLeftIcon className="h-5 w-5" /></Button>
                            {mode === "upgrade" && <Button type="button" onClick={onSaveExit}>Save & Exit</Button>}
                            <Button type="button" onClick={handleProceed} className="flex-1 disabled:opacity-50 disabled:cursor-not-allowed" style={{ backgroundColor: brandColor, color: contrastColor }} disabled={!isStepValid}>
                                {proceedButtonText}
                            </Button>
                        </div>
                    ...
                    <>
                                    ...
                                    {isLoadingCategories ? (
                                        ...
                                        availableCategories.map((cat) => (
                                            <div key={cat.id} onClick={() => {
                                                ...
                                                const updated = selected.includes(cat.id) ? selected.filter(id => id !== cat.id) : [...selected, cat.id];
                                                handleChange({ target: { name: "categories", value: updated } });
                                            }}
                                            // FIXED: className syntax with template literal
                                            className={`p-2 cursor-pointer hover:bg-gray-100 ${formData.categories?.includes(cat.id) ? "bg-red-100" : ""}`}>
                                                {cat.title}
                                            </div>
                                        ))
                                    )}
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                            {(formData.categories || []).map((catId) => {
                                ...
                                return (
                                    <span key={catId} className="px-3 py-1 rounded-full text-sm flex items-center" style={{ backgroundColor: brandColor, color: contrastColor }}>
                                        {category ? category.title : '...'}
                                        <button type="button" onClick={() => handleRemoveCategory(catId)} className="ml-2">
                        ...
                        <Input name="whatsapp" placeholder="WhatsApp Link (Optional)" value={formData.whatsapp || ""} onChange={handleChange} />
                        <Input name="instagram" placeholder="Instagram Link (Optional)" value={formData.instagram || ""} onChange={handleChange} />
                        <Input name="facebook" placeholder="Facebook Link (Optional)" value={formData.facebook || ""} onChange={handleChange} />
                        <Input name="twitter" placeholder="Twitter Link (Optional)" value={formData.twitter || ""} onChange={handleChange} />
            ...
            <EnterEmailModal isOpen={showEnterEmail} onEmailSubmit={handleEmailSubmit} onClose={() => setShowEnterEmail(false)} brandColor={brandColor} contrastColor={contrastColor} isProcessing={processing.send} />
            <OtpInputModal isOpen={showOtp} onOtpConfirm={handleOtpConfirm} onClose={() => setShowOtp(false)} email={resetEmail} brandColor={brandColor} contrastColor={contrastColor} isProcessing={processing.verify} />
            <SetNewPasswordModal isOpen={showSetNewPassword} onSetPassword={handleNewPassword} onClose={() => setShowSetNewPassword(false)} brandColor={brandColor} contrastColor={contrastColor} isProcessing={processing.reset} />
        </div>
    );
};
